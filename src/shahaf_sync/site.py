    ...
    *,
    ...
    ui_translations = {
        "en": {
            ...
            "alarmTitle": "My alarm", "alarmScheduled": "Scheduled for", "alarmNotScheduled": "Not scheduled", "alarmButton": "Cancel / move my next alarm", "alarmInstruction": "This changes only this schedule’s next alarm. Restore puts it back to the correct original wake time, before any changes. Run the Shahaf Shortcut to apply it immediately; the full schedule sync continues in the background.",
            "cancelTodayAlarm": "Cancel my next alarm", "moveAlarmTo": "Move my next alarm to", "moveAlarm": "Move alarm", "restoreAlarm": "Restore correct time",
        ...
        "he": {
            ...
            "alarmTitle": "ההתראה שלי", "alarmScheduled": "מתוזמנת לשעה", "alarmNotScheduled": "אין התראה מתוזמנת", "alarmButton": "ביטול / שינוי ההתראה הבאה שלי", "alarmInstruction": "פעולה זו משנה רק את ההתראה הבאה של המערכת הזו. השחזור מחזיר אותה לשעת ההשכמה המקורית והנכונה, לפני כל שינוי. יש להפעיל את קיצור הדרך של שחף כדי להחיל מיד; סנכרון המערכת המלא ימשיך ברקע.",
            "cancelTodayAlarm": "ביטול ההתראה הבאה שלי", "moveAlarmTo": "שינוי ההתראה הבאה שלי לשעה", "moveAlarm": "שינוי ההתראה", "restoreAlarm": "שחזור לשעה הנכונה",
...
function renderFullDay(targetDate, direction = "") {{
  const dates = scheduleDates();
  if (!dates.length) {{
    document.getElementById("schedule-periods").innerHTML = `<div class="empty-day">The full schedule will appear after a successful sync.</div>`;
    return;
  }}
  const selected = dates.includes(targetDate) ? targetDate : dates[0];
  selectedDayDate = selected;
  renderDayPicker(selected);
  const asyncEvent = asyncEventFor(selected);
  const notice = document.getElementById("day-notice");
  if (notice) {{
    notice.hidden = !asyncEvent;
    notice.innerHTML = asyncEvent ? `<strong>${{escapeHtml(tr("asyncDay"))}}</strong><span>${{escapeHtml(tr("asyncDayDetail"))}}</span>` : "";
  }}
  const items = schedule.filter((item) => item.date === selected);
  const byPeriod = Object.fromEntries(items.map((item) => [item.period, item]));
  const cancelledByOccurrence = new Set(
    changes
      .filter((change) => change.kind === "cancelled")
      .map((change) => `${{change.date}}:${{change.period}}`)
  );
  const lessonPeriods = items.map((item) => Number(item.period)).filter((period) => Number.isInteger(period));
  const firstLessonPeriod = lessonPeriods.length ? Math.min(...lessonPeriods) : null;
  const lastLessonPeriod = lessonPeriods.length ? Math.max(...lessonPeriods) : null;
  const day = schoolDate(selected);
  document.getElementById("selected-day-title").textContent = dateFormatter.format(day);
  ...
  document.getElementById("schedule-periods").innerHTML = periods.map((slot) => {{
    ...
    const emptyKind = outsideSchoolDay ? "no-lesson" : "gap";
    const accent = palette[slot.period % palette.length];
    const mainMarkup = item
      ? `<strong>${{escapeHtml(item.subject)}}</strong><span>${{escapeHtml([item.teacher, item.room ? `${{tr("room")}} ${{item.room}}` : ""].filter(Boolean).join(" · ") || tr("lesson"))}}</span>`
      : cancelled
        ? ""
        : `<span class="gap-label" data-empty-kind="${{emptyKind}}">${{tr(emptyKind === "no-lesson" ? "noLesson" : "gap")}}</span>`;
    return `<article class="period-row ${{rowClass}}"${{ariaLabel}} style="--slot-accent:${{accent}}"><div class="period-main">${{mainMarkup}}</div><div class="period-info" dir="ltr"><strong><bdi>${{slot.period}}</bdi></strong><span class="period-range" dir="ltr"><bdi>${{formatTime(slot.start)}}</bdi>–<bdi>${{formatTime(slot.end)}}</bdi></span></div></article>`;
  }}).join("");
  const surface = document.getElementById("full-day-content");
  if (direction) animateElement(surface, direction === "next" ? "day-surface-enter-next" : "day-surface-enter-previous");
}}
