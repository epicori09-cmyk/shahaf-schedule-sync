...
function profileView(row, origin) {
  return {
    ...
    wake_url: `${origin}/students/${row.public_id}/wake.json`,
...
<script>
(function(){"use strict";var csrf="";var $=function(id){return document.getElementById(id)};var loginScreen=$("loginScreen"),dashboard=$("dashboard"),logoutBtn=$("logoutBtn");var escapeHtml=function(value){return String(value==null?"":value).replace(/[&<>"']/g,function(ch){return {"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[ch]})};var formatDate=function(value){if(!value)return "—";var d=new Date(value);return isNaN(d.getTime())?String(value):d.toLocaleString([], {dateStyle:"medium",timeStyle:"short"})};var toast=function(message,bad){var el=$("toast");el.textContent=message;el.className="toast"+(bad?" bad":"");clearTimeout(toast.timer);toast.timer=setTimeout(function(){el.className="toast hidden"},4200)};
...
function renderHealth(health){var cards=[{label:"Active profiles",value:String(health.profileCount),meta:String(health.totalProfiles)+" total in D1",cls:""},{label:"Main schedule",value:statusText(health.main),meta:health.main.available?"Updated "+formatDate(health.main.generated_at):health.main.error,cls:statusClass(health.main)},{label:"Ya1 schedule",value:statusText(health.ya1),meta:health.ya1.available?"Updated "+formatDate(health.ya1.generated_at):health.ya1.error,cls:statusClass(health.ya1)},{label:"GitHub workflow",value:health.workflow.available?(health.workflow.conclusion||health.workflow.status||"Ready"):"Unavailable",meta:health.workflow.available?"Checked "+formatDate(health.workflow.updated_at):health.workflow.error,cls:health.workflow.available&&health.workflow.conclusion==="failure"?"bad":""}];$("healthGrid").innerHTML=cards.map(function(card){return '<article class="health-card"><div class="health-label"><span>'+escapeHtml(card.label)+'</span><span class="status '+card.cls+'">'+escapeHtml(statusText(card))+'</span></div><div class="health-value">'+escapeHtml(card.value)+'</div><div class="health-meta">'+escapeHtml(card.meta)+'</div></article>'}).join("")}
function renderProfiles(profiles){var active=profiles.filter(function(p){return p.active}).length;$("profileSummary").textContent=profiles.length?active+" active · "+(profiles.length-active)+" disabled":"No profiles yet";if(!profiles.length){$("profiles").innerHTML='<div class="empty">No managed profiles yet. Import the first verified timetable above.</div>';return}$("profiles").innerHTML=profiles.map(function(p){var state=p.active?"Active":"Disabled";return '<article class="profile-card"><div class="profile-top"><div><div class="profile-name">'+escapeHtml(p.name)+'</div><div class="profile-id">'+escapeHtml(p.public_id)+'</div></div><span class="status '+(p.active?"":"warn")+'">'+state+'</span></div><div class="profile-details"><div><strong>Published:</strong> '+escapeHtml(p.last_publish_status||"never")+'</div><div><strong>Updated:</strong> '+escapeHtml(formatDate(p.updated_at))+'</div><div><strong>Alarm:</strong> <code>'+escapeHtml(p.alarm_label)+'</code></div></div><div class="profile-links"><a href="'+escapeHtml(p.page_url)+'" target="_blank" rel="noreferrer">Open schedule ↗</a><a href="'+escapeHtml(p.wake_url)+'" target="_blank" rel="noreferrer">Open wake JSON ↗</a></div><div class="profile-actions"><button class="button" data-view="'+escapeHtml(p.id)+'">View details</button><button class="button danger" data-disable="'+escapeHtml(p.id)+'" '+(p.active?"":"disabled")+'>Disable</button></div></article>'}).join("");document.querySelectorAll("[data-view]").forEach(function(button){button.onclick=function(){viewProfile(button.getAttribute("data-view"))}});document.querySelectorAll("[data-disable]").forEach(function(button){button.onclick=function(){disableProfile(button.getAttribute("data-disable"))}})}
