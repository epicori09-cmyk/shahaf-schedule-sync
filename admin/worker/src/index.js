...
(function(){"use strict";
  ...
  var globalHtml=function(settings){settings=settings||{};return '<div class="alarm-settings-card"><h3>Managed-profile defaults</h3><div class="alarm-form-grid">'+checkbox("Enable managed alarm synchronization","alarmGlobalEnabled",settings.enabled!==false)+field("Wake buffer (minutes)","alarmGlobalBuffer","number",valueOr(settings.wake_buffer_minutes,75))+field("Fallback wake time","alarmGlobalFallback","time",valueOr(settings.fallback_wake_time,"07:15"))+field("Minimum wake time (optional)","alarmGlobalMin","time",settings.min_wake_time,"wide")+field("Maximum wake time (optional)","alarmGlobalMax","time",settings.max_wake_time,"wide")+select("Rounding","alarmGlobalRound",valueOr(settings.round_to_minutes,1),[["1","No rounding"],["5","5 minutes"],["10","10 minutes"],["15","15 minutes"]])+select("When source is stale","alarmGlobalStale",valueOr(settings.stale_policy,"leave"),[["leave","Leave alarm unchanged"],["set_fixed","Use fixed fallback"]])+select("When no lessons are confirmed","alarmGlobalNoLessons",valueOr(settings.no_lessons_policy,"clear"),[["clear","Clear primary alarm"],["leave","Leave alarm unchanged"]])+field("Alarm label template","alarmGlobalLabel","text",valueOr(settings.label_template,"Shahaf"),"wide")+field("Transit early-arrival margin","alarmGlobalTransitMargin","number",valueOr(settings.transit_min_arrival_margin,5))+field("Extra transit walking buffer","alarmGlobalTransitWalk","number",valueOr(settings.transit_walk_buffer_minutes,0))+ '</div><p class="alarm-help">These defaults apply only to managed student profiles. Existing Ya‑1 and Ya‑2 remain isolated. Use <code>{public_id}</code> or <code>{profile_id}</code> in a label template if needed.</p><div class="alarm-actions"><button id="alarmSaveGlobal" class="button primary">Save global defaults</button><button id="alarmPreviewGlobal" class="button">Preview effective policy</button><button id="alarmRollbackGlobal" class="button">Rollback last global change</button></div><div id="alarmGlobalPreview" class="alarm-preview hidden"></div></div>'};
  var bulkHtml=function(){return '<div class="alarm-bulk-card"><h3>Bulk command center</h3><p class="alarm-help">Select managed profiles below. Commands are applied by their next Shortcut run.</p><div class="alarm-form-grid">'+select("Action","alarmBulkAction","pause",[["pause","Pause sync"],["resume","Resume sync"],["reset","Reset profile settings"],["set","Set alarm"],["clear","Clear primary alarm"],["leave","Leave unchanged"]],"wide")+field("Target date","alarmBulkDate","date",dateInput())+field("Wake time (only for set)","alarmBulkWake","time","07:15")+field("Reason for force action","alarmBulkReason","text","","wide")+checkbox("Force this action through stale/unsafe data","alarmBulkForce",false)+'</div><div class="alarm-actions"><button id="alarmBulkPreview" class="button">Preview selected</button><button id="alarmBulkApply" class="button danger">Apply to selected</button></div><div id="alarmBulkResult" class="alarm-preview hidden"></div></div>'};
  ...
  var renderProfiles=function(profiles){return (profiles||[]).map(function(item){var live=item.live||{};var effective=item.effective||{};var override=item.override;var state=item.active?"Active":"Disabled";var overrideText=override?(override.action+" on "+override.target_date):"None";var routeIds=item.settings&&item.settings.transit_route_preference&&Array.isArray(item.settings.transit_route_preference.route_ids)?item.settings.transit_route_preference.route_ids.join(","):"";return '<article class="alarm-profile"><div class="alarm-profile-top"><label><input class="alarm-select" type="checkbox" value="'+escapeHtml(item.id)+'"> <span class="alarm-profile-name">'+escapeHtml(item.name)+'</span></label><span class="alarm-badge">'+state+'</span></div><div class="alarm-profile-id">'+escapeHtml(item.public_id)+'</div><div class="alarm-profile-summary"><div class="alarm-stat"><span>Next wake</span><strong>'+escapeHtml(liveText(live))+'</strong></div><div class="alarm-stat"><span>Action</span><strong>'+escapeHtml(live.available?(live.shortcut_action||"set"):"unknown")+'</strong></div><div class="alarm-stat"><span>Label</span><strong>'+escapeHtml(effective.alarm_label||"Shahaf")+'</strong></div><div class="alarm-stat"><span>Override</span><strong>'+escapeHtml(overrideText)+'</strong></div></div><details class="alarm-profile-details"><summary>Profile alarm controls</summary><div class="alarm-form-grid">'+checkbox("Enable this profile’s alarm sync","alarmEnabled_"+item.id,effective.enabled!==false)+field("Wake buffer (blank inherits global)","alarmBuffer_"+item.id,"number",valueOr(item.settings.wake_buffer_minutes,""))+field("Alarm label override (blank inherits)","alarmLabel_"+item.id,"text",valueOr(item.settings.alarm_label,""),"wide")+field("Minimum wake time (optional)","alarmMin_"+item.id,"time",valueOr(item.settings.min_wake_time,""))+field("Maximum wake time (optional)","alarmMax_"+item.id,"time",valueOr(item.settings.max_wake_time,""))+select("Rounding","alarmRound_"+item.id,valueOr(item.settings.round_to_minutes,""),[["","Inherit global"],["1","No rounding"],["5","5 minutes"],["10","10 minutes"],["15","15 minutes"]])+select("Stale policy","alarmStale_"+item.id,valueOr(item.settings.stale_policy,""),[["","Inherit global"],["leave","Leave unchanged"],["set_fixed","Fixed fallback"]])+select("No lessons","alarmNoLessons_"+item.id,valueOr(item.settings.no_lessons_policy,""),[["","Inherit global"],["clear","Clear alarm"],["leave","Leave unchanged"]])+field("Transit early-arrival margin","alarmTransitMargin_"+item.id,"number",valueOr(item.settings.transit_min_arrival_margin,""))+field("Extra transit walking buffer","alarmTransitWalk_"+item.id,"number",valueOr(item.settings.transit_walk_buffer_minutes,""))+field("Pinned route IDs (optional)","alarmTransitRoutes_"+item.id,"text",routeIds,"wide")+'</div><div class="alarm-actions"><button class="button primary alarm-save-profile" data-id="'+escapeHtml(item.id)+'">Save profile controls</button><button class="button alarm-reset-profile" data-id="'+escapeHtml(item.id)+'">Reset to global</button><button class="button alarm-history-profile" data-id="'+escapeHtml(item.id)+'">Show history</button><button class="button alarm-rollback-profile" data-id="'+escapeHtml(item.id)+'">Rollback</button></div><div class="alarm-form-grid">'+field("One-time command date","alarmCommandDate_"+item.id,"date",dateInput(override&&override.target_date))+select("One-time action","alarmCommandAction_"+item.id,"set",[["set","Set alarm"],["clear","Clear primary alarm"],["leave","Leave unchanged"]])+field("One-time wake time","alarmCommandWake_"+item.id,"time",override&&override.wake_at?new Date(override.wake_at).toTimeString().slice(0,5):"07:15")+field("Force reason","alarmCommandReason_"+item.id,"text","","wide")+checkbox("Force through stale/unsafe data","alarmCommandForce_"+item.id,false)+'</div><div class="alarm-actions"><button class="button danger alarm-command-profile" data-id="'+escapeHtml(item.id)+'">Queue one-time command</button></div><div id="alarmHistory_'+escapeHtml(item.id)+'" class="alarm-audit hidden"></div></details></article>'}).join("")||'<div class="empty">No managed profiles available.</div>'};
  ...
  var load=async function(){try{model.data=await api("/api/dashboard");render();bind()}catch(error){if(!/login required|session expired|HTTP 401/.test(error.message))toast(error.message,true)}};
