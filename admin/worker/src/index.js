...
// These are deliberately small, human-facing choices. They are stored with
// the profile so the exam/schedule pipeline can use the student's actual
// English, Math, and major tracks without making the operator edit JSON.
const PROFILE_LEVELS = new Set(["3", "4", "5", "5_accelerated"]);
...
function validatePackage(input) {
  ...
  for (const field of ["english_level", "math_level"]) {
    const value = shahaf?.[field];
    if (value !== null && value !== undefined && value !== "" && !PROFILE_LEVELS.has(String(value))) {
      errors.push(`shahaf.${field} must be 3, 4, 5, or 5_accelerated`);
    }
  }
  if (shahaf?.majors !== undefined && (!Array.isArray(shahaf.majors) || shahaf.majors.some((value) => typeof value !== "string" || !value.trim()))) {
    errors.push("shahaf.majors must be a list of non-empty strings");
  }
  ...
  return { package: {
    ...
    shahaf: { class_id: String(shahaf.class_id), class_number: Number(shahaf.class_number), shared_subjects: Array.isArray(shahaf.shared_subjects) ? shahaf.shared_subjects : [], selectors: Array.isArray(shahaf.selectors) ? shahaf.selectors : [], exam_terms: Array.isArray(shahaf.exam_terms) ? shahaf.exam_terms : [], exam_exact_terms: Array.isArray(shahaf.exam_exact_terms) ? shahaf.exam_exact_terms : [], english_level: shahaf.english_level || null, math_level: shahaf.math_level || null, majors: Array.isArray(shahaf.majors) ? shahaf.majors.map((value) => String(value).trim()).filter(Boolean) : [] },
...
function applyProfileChoices(packageInput, body) {
  if (!packageInput || typeof packageInput !== "object" || Array.isArray(packageInput)) return packageInput;
  const shahaf = { ...(packageInput.shahaf || {}) };
  for (const field of ["english_level", "math_level"]) {
    if (Object.prototype.hasOwnProperty.call(body, field)) shahaf[field] = body[field] || null;
  }
  if (Object.prototype.hasOwnProperty.call(body, "majors")) {
    shahaf.majors = Array.isArray(body.majors) ? body.majors : [];
  }
  return { ...packageInput, shahaf };
}
...
function profileView(row, origin, shortcutOrigin) {
  ...
  return {
    ...
    wake_url: shortcutUrl,
    public_wake_url: wakeUrl,
...
<section id="healthGrid" class="health-grid" aria-label="System health"><div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div></section>
 <div class="workspace"><section id="importPanel" class="panel panel-pad"><div class="section-head"><div><h2>Add a student</h2><p>Enter the student details, choose their tracks, then paste or upload the timetable JSON.</p></div><span class="status">Private</span></div><div class="field"><label for="studentName">Student name</label><input id="studentName" class="input" placeholder="Example: Alex Cohen" autocomplete="off"><span class="helper">Used only in this private dashboard.</span></div><div class="field"><label for="classNumber">יא class number</label><input id="classNumber" class="input" type="number" min="1" max="50" step="1" required placeholder="Example: 2"><span class="helper">Enter the visible class number. The Shahaf ID is filled automatically.</span></div><div class="edit-grid"><div class="field"><label for="englishLevel">English level</label><select id="englishLevel" class="input"><option value="">Not set</option><option value="3">3 points</option><option value="4">4 points</option><option value="5">5 points</option><option value="5_accelerated">5 points · accelerated</option></select></div><div class="field"><label for="mathLevel">Math level</label><select id="mathLevel" class="input"><option value="">Not set</option><option value="3">3 points</option><option value="4">4 points</option><option value="5">5 points</option><option value="5_accelerated">5 points · accelerated</option></select></div></div><div class="field"><span class="field-label">Majors / מגמות</span><div class="edit-grid"><label><input type="checkbox" name="major" value="physics"> Physics · פיזיקה</label><label><input type="checkbox" name="major" value="chemistry"> Chemistry · כימיה</label><label><input type="checkbox" name="major" value="biology"> Biology · ביולוגיה</label><label><input type="checkbox" name="major" value="computer_science"> Computer science · מדעי המחשב</label><label><input type="checkbox" name="major" value="cyber"> Cyber · סייבר</label><label><input type="checkbox" name="major" value="diplomacy"> Diplomacy · דיפלומטיה</label><label><input type="checkbox" name="major" value="geography"> Geography · גיאוגרפיה</label><label><input type="checkbox" name="major" value="arabic"> Arabic · ערבית</label><label><input type="checkbox" name="major" value="mizrahnut"> Oriental studies · מזרחנות</label><label><input type="checkbox" name="major" value="social_sciences"> Social sciences · מדעי החברה</label><label><input type="checkbox" name="major" value="business_management"> Business management · ניהול עסקי</label><label><input type="checkbox" name="major" value="communications_new_media"> Communications, society & new media · תקשורת חברה וניו מדיה</label><label><input type="checkbox" name="major" value="french"> French · צרפתית</label><label><input type="checkbox" name="major" value="spanish"> Spanish · ספרדית</label><label><input type="checkbox" name="major" value="russian"> Russian · רוסית</label><label><input type="checkbox" name="major" value="art"> Art · אומנות</label><label><input type="checkbox" name="major" value="extended_history"> Extended history · היסטוריה מורחב</label></div><span class="helper">These are the elective tracks visible in the Shahaf class timetable. Choose every track this student actually takes.</span></div><div class="field"><label for="payload">Timetable package</label><textarea id="payload" class="textarea" spellcheck="false" placeholder="Paste the complete JSON object here…"></textarea></div><div class="import-actions"><label class="button file-button">Load .json file<input id="file" type="file" accept=".json,application/json"></label><button id="importBtn" class="button primary">Add and publish</button></div><pre id="result" class="result" aria-live="polite"></pre></section>
 <aside class="panel panel-pad"><div class="section-head"><div><h2>What you can do</h2><p>Keep each student’s details easy to maintain.</p></div></div><ul class="quick-list"><li><b>01</b><span>Add a student with their class, English level, Math level, and majors.</span></li><li><b>02</b><span>Edit timetable rows as simple day-by-day cards.</span></li><li><b>03</b><span>Open the published schedule in English or Hebrew.</span></li><li><b>04</b><span>Copy the student’s private Shortcut endpoint.</span></li></ul></aside></div>
 <section id="editPanel" class="panel panel-pad hidden"><div class="section-head"><div><h2>Edit student</h2><p>Update the student’s details or edit the timetable in day-by-day cards.</p></div><button id="cancelEdit" class="button">Close</button></div><div class="edit-grid"><div class="field"><label for="editName">Student name</label><input id="editName" class="input" autocomplete="off"></div><div class="field"><label for="editClassNumber">יא class number</label><input id="editClassNumber" class="input" type="number" min="1" max="50" step="1"></div><div class="field"><label for="editEnglishLevel">English level</label><select id="editEnglishLevel" class="input"><option value="">Not set</option><option value="3">3 points</option><option value="4">4 points</option><option value="5">5 points</option><option value="5_accelerated">5 points · accelerated</option></select></div><div class="field"><label for="editMathLevel">Math level</label><select id="editMathLevel" class="input"><option value="">Not set</option><option value="3">3 points</option><option value="4">4 points</option><option value="5">5 points</option><option value="5_accelerated">5 points · accelerated</option></select></div></div><div class="field"><span class="field-label">Majors / מגמות</span><div class="edit-grid"><label><input type="checkbox" name="edit-major" value="physics"> Physics · פיזיקה</label><label><input type="checkbox" name="edit-major" value="chemistry"> Chemistry · כימיה</label><label><input type="checkbox" name="edit-major" value="biology"> Biology · ביולוגיה</label><label><input type="checkbox" name="edit-major" value="computer_science"> Computer science · מדעי המחשב</label><label><input type="checkbox" name="edit-major" value="cyber"> Cyber · סייבר</label><label><input type="checkbox" name="edit-major" value="diplomacy"> Diplomacy · דיפלומטיה</label><label><input type="checkbox" name="edit-major" value="geography"> Geography · גיאוגרפיה</label><label><input type="checkbox" name="edit-major" value="arabic"> Arabic · ערבית</label><label><input type="checkbox" name="edit-major" value="mizrahnut"> Oriental studies · מזרחנות</label><label><input type="checkbox" name="edit-major" value="social_sciences"> Social sciences · מדעי החברה</label><label><input type="checkbox" name="edit-major" value="business_management"> Business management · ניהול עסקי</label><label><input type="checkbox" name="edit-major" value="communications_new_media"> Communications, society & new media · תקשורת חברה וניו מדיה</label><label><input type="checkbox" name="edit-major" value="french"> French · צרפתית</label><label><input type="checkbox" name="edit-major" value="spanish"> Spanish · ספרדית</label><label><input type="checkbox" name="edit-major" value="russian"> Russian · רוסית</label><label><input type="checkbox" name="edit-major" value="art"> Art · אומנות</label><label><input type="checkbox" name="edit-major" value="extended_history"> Extended history · היסטוריה מורחב</label></div></div><div class="field"><label for="editPayload">Timetable package</label><textarea id="editPayload" class="textarea" spellcheck="false"></textarea><span class="helper">The editor above shows each day and period as a card. Keep periods 0–13 and save when finished.</span></div><div class="import-actions"><span id="editId" class="helper"></span><button id="saveEdit" class="button primary">Save changes</button></div><pre id="editResult" class="result" aria-live="polite"></pre></section>
...
function renderProfiles(profiles){var active=profiles.filter(function(p){return p.active}).length;$("profileSummary").textContent=profiles.length?active+" active · "+(profiles.length-active)+" disabled":"No students yet";if(!profiles.length){$("profiles").innerHTML='<div class="empty">No students yet. Add the first timetable above.</div>';return}$("profiles").innerHTML=profiles.map(function(p){var state=p.active?"Active":"Disabled";return '<article class="profile-card"><div class="profile-top"><div><div class="profile-name">'+escapeHtml(p.name)+'</div><div class="profile-id">'+escapeHtml(p.public_id)+'</div></div><span class="status '+(p.active?"":"warn")+'">'+state+'</span></div><div class="profile-details"><div><strong>Published:</strong> '+escapeHtml(p.last_publish_status||"never")+'</div><div><strong>Updated:</strong> '+escapeHtml(formatDate(p.updated_at))+'</div></div><div class="profile-links"><a href="'+escapeHtml(p.page_url+"?lang=en")+'" target="_blank" rel="noreferrer">English schedule ↗</a><a href="'+escapeHtml(p.page_url+"?lang=he")+'" target="_blank" rel="noreferrer">Hebrew schedule ↗</a><a href="'+escapeHtml(p.wake_url)+'" target="_blank" rel="noreferrer">Shortcut endpoint ↗</a></div><div class="profile-actions"><button class="button" data-view="'+escapeHtml(p.id)+'">Edit schedule</button></div></article>'}).join("");document.querySelectorAll("[data-view]").forEach(function(button){button.onclick=function(){viewProfile(button.getAttribute("data-view"))}})}
...
(function(){"use strict";
  ...
  window.openManagedProfileEditor=async function(id){try{var data=await callApi("/api/profiles/"+encodeURIComponent(id));var pkg=data.package||{};byId("editId").textContent="Editing "+data.public_id;byId("editId").setAttribute("data-profile-id",id);byId("editName").value=data.name||(pkg.student&&pkg.student.name)||"";byId("editClassNumber").value=(pkg.shahaf&&pkg.shahaf.class_number)||"";byId("editEnglishLevel").value=(pkg.shahaf&&pkg.shahaf.english_level)||"";byId("editMathLevel").value=(pkg.shahaf&&pkg.shahaf.math_level)||"";var editMajors=(pkg.shahaf&&Array.isArray(pkg.shahaf.majors))?pkg.shahaf.majors:[];document.querySelectorAll('input[name="edit-major"]').forEach(function(input){input.checked=editMajors.indexOf(input.value)>=0});byId("editPayload").value=JSON.stringify(pkg,null,2);byId("editResult").textContent="";byId("editPanel").classList.remove("hidden");byId("editPanel").scrollIntoView({behavior:"smooth",block:"start"})}catch(error){toast(error.message,true)}};
  window.saveManagedProfile=async function(){var id=byId("editId").getAttribute("data-profile-id");if(!id)return;var button=byId("saveEdit");button.disabled=true;byId("editResult").textContent="Validating and saving…";try{var pkg=JSON.parse(byId("editPayload").value);pkg.student=pkg.student||{};pkg.student.name=byId("editName").value.trim();pkg.shahaf=pkg.shahaf||{};pkg.shahaf.class_number=Number(byId("editClassNumber").value);pkg.shahaf.english_level=byId("editEnglishLevel").value||null;pkg.shahaf.math_level=byId("editMathLevel").value||null;pkg.shahaf.majors=Array.from(document.querySelectorAll('input[name="edit-major"]:checked')).map(function(input){return input.value});await callApi("/api/profiles/"+encodeURIComponent(id),{method:"PATCH",body:JSON.stringify({name:pkg.student.name,class_number:pkg.shahaf.class_number,package:pkg})});byId("editResult").textContent="Saved. Publish queued.";toast("Profile saved; publish queued");setTimeout(function(){byId("editPanel").classList.add("hidden");byId("refreshBtn").click()},700)}catch(error){byId("editResult").textContent=error.message;toast("Profile was not saved",true)}finally{button.disabled=false}};
  ...
  var enhance=function(){document.querySelectorAll("[data-view]").forEach(function(oldButton){var id=oldButton.getAttribute("data-view");var button=document.createElement("button");button.className=oldButton.className;button.setAttribute("data-profile-id",id);button.textContent="Edit schedule";button.onclick=function(){return window.openManagedProfileEditor(id)};oldButton.replaceWith(button)})};
  ...
  var observer=new MutationObserver(enhance);var profiles=byId("profiles");if(profiles)observer.observe(profiles,{childList:true});enhance();byId("cancelEdit").onclick=function(){byId("editPanel").classList.add("hidden")};byId("saveEdit").onclick=window.saveManagedProfile;
})();</script>`;
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/profiles/import" && request.method === "POST") {
      ...
      const packageInput = body.package && typeof body.package === "object" && !Array.isArray(body.package)
        ? applyProfileChoices({ ...body.package, shahaf: { ...(body.package.shahaf || {}), class_id: classId, class_number: classNumber } }, body)
        : body.package;
    ...
    if (match && request.method === "PATCH") {
      ...
      const packageInput = applyProfileChoices({
        ...incoming,
