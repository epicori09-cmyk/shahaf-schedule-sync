...
class AdminContractTests(unittest.TestCase):
    def test_worker_has_private_routes_and_security_controls(self) -> None:
        ...
        for route in ("/api/login", "/api/logout", "/api/session", "/api/classes", "/api/profiles", "/api/profiles/import", "/enable$/", "/disable$/", "request.method === \"DELETE\"", "/internal/profiles", "/internal/alarm-commands/ack", "public\\/profiles\\/", "publicOriginOK", "publicCorsHeaders", "alarm-command", "/api/alarm-settings", "/api/alarm-preview", "/api/alarm-bulk", "/alarm-history$/", "alarm-settings/rollback", "const publish ="):
        ...
        for control in ("PBKDF2", "HttpOnly", "SameSite=Strict", "X-CSRF-Token", "ADMIN_ORIGIN", "rateLimit", "Permanently delete this profile", "Alarm control center", "Force this change (advanced)", "Restore this version", "route_alternatives", "published_at", "action must be clear or set", "future time today", "No alarm can be moved on Friday or Saturday", "israelIsWeekend", "WHERE public_id=?1 AND active=1", "Student self-service alarm change", 'source: "public-profile"'):
