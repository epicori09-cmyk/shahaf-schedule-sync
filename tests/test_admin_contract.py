...
class AdminContractTests(unittest.TestCase):
    def test_worker_has_private_routes_and_security_controls(self) -> None:
        ...
        for route in ("/api/login", "/api/logout", "/api/session", "/api/classes", "/api/profiles", "/api/profiles/import", "/enable$/", "/disable$/", "request.method === \"DELETE\"", "/internal/profiles", "/internal/alarm-commands/ack", "public\\/profiles\\/", "publicOriginOK", "publicCorsHeaders", "publicWakeEndpoint", "wake\\.json", "applyPublicAlarmOverride", "alarm-command", "/api/alarm-settings", "/api/alarm-preview", "/api/alarm-bulk", "/alarm-history$/", "alarm-settings/rollback", "const publish ="):
